...
main (void)
{
  math_error_subtyping_init_module();

  {
    ...
    if (cce_location(L)) {
      ...
      if (my_condition_is_math_error_subtype(cce_condition(L))) {
	CCLIB_PC(my_condition_math_error_subtype_t, C, cce_condition(L));
	...
	exit(EXIT_FAILURE);
      ...
      cce_raise(L, my_condition_new_math_error_subtype(L, 123));
