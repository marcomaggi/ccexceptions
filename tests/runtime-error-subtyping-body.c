...
void
my_condition_init_runtime_error_subtype (my_condition_runtime_error_subtype_t * C, int the_data)
/* Here we call the parent's initialisation function; then we initialise
   the fields of this type. */
{
  cce_condition_init_runtime_error(&(C->parent));
  *(C->data) = the_data;
}

cce_condition_t const *
my_condition_new_runtime_error_subtype (cce_destination_t L, int the_data)
