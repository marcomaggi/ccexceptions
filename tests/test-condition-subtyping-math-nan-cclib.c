...
main (void)
{
  math_nan_cclib_subtyping_init_module();

  {
    ...
    if (cce_location(L)) {
      ...
      if (cclib_is(cclib_exceptional_condition_object_type(my_math_nan_subtype))(cce_condition(L))) {
	CCLIB_PC(cclib_exceptional_condition_object_type(my_math_nan_subtype), C, cce_condition(L));
	...
	exit(EXIT_FAILURE);
      ...
      if (cclib_is(cclib_exceptional_condition_object_type(cce_math_nan))(cce_condition(L))) {
	...
	exit(EXIT_FAILURE);
      ...
      cce_raise(L, cclib_new(cclib_exceptional_condition_object_type(my_math_nan_subtype))(L, 123));
