...
main (void)
{
...

  {
    ...
    if (cce_location(L)) {
      ...
      if (my_condition_is_error_subtype(cce_condition(L))) {
	CCE_PC(my_condition_error_subtype_t, C, cce_condition(L));
	...
	exit(EXIT_FAILURE);
      ...
      cce_raise(L, my_condition_new_error_subtype(L, 123));
