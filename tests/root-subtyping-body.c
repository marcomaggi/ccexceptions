...
void
my_condition_init_root_subtype (my_condition_root_subtype_t * C, int the_data)
/* Here we call the parent's initialisation function; then we initialise
   the fields of this type. */
{
  cce_condition_init_root(&(C->parent));
  *(C->data) = the_data;
}

cce_condition_t const *
my_condition_new_root_subtype (cce_destination_t L, int the_data)
   ...
   descriptor field; then we call the initialisation function. */
{
  ...
  my_condition_init_root_subtype(C, the_data);
...
{
  cce_descriptor_set_root_parent(&my_descriptor_root_subtype_stru.descriptor);
}
