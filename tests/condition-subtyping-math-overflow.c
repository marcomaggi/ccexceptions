...
main (void)
{
  math_overflow_subtyping_init_module();

  {
    ...
    if (cce_location(L)) {
      ...
      if (my_condition_is_math_overflow_subtype(cce_condition(L))) {
	CCLIB_PC(my_condition_math_overflow_subtype_t, C, cce_condition(L));
	...
	exit(EXIT_FAILURE);
      ...
      if (cce_condition_is_math_overflow(cce_condition(L))) {
	...
	exit(EXIT_FAILURE);
      ...
      cce_raise(L, my_condition_new_math_overflow_subtype(L, 123));
